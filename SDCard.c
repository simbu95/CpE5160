...
************************************************************************/
uint8_t Send_Command(volatile SPI_t *SPI_addr, uint8_t cmnd, uint32_t argum)
{
 ...
 {
   SPI_send=cmnd | 0x40;
   error_flag=SPI_Transmit(SPI_addr,SPI_send);
...
CAUTION: Used to capture SD card response
         nCS0 must be set/cleared outside of this routine 
************************************************************************/
uint8_t Receive_Response(volatile SPI_t *SPI_addr, uint8_t num_bytes, uint8_t * valout)
{
   ...
   do
   {
      error_flag=SPI_Receive(SPI_addr, &SPI_return);
      index++;
   ...
   error_flag=SPI_Receive(SPI_addr, &SPI_return);  // send 8 more clock cycles to complete read
   return return_val;
}
...
CAUTION: Used to capture SD card response
         nCS0 must be set/cleared outside of this routine 
************************************************************************/
uint8_t Read_Block(volatile SPI_t *SPI_addr, uint16_t num_bytes, uint8_t * array_out)
{
   ...
   else
   {
     if (SPI_return==0x00)
     {
        ...
        else if(SPI_return==0xfe)
        {
          ...
          error_flag=SPI_Receive(SPI_addr, &SPI_return); // discard byte 1 of CRC16	
          error_flag=SPI_Receive(SPI_addr, &SPI_return); // discard byte 2 of CRC16
        }
	    else
	    {
...
uint8_t SD_Card_Init(volatile SPI_t *SD_Port)
{
   ...
   do 
   {
		  ...
		  for(i=0;i<10;i++)
		  {
			  SPI_return=SPI_Transmit(SD_Port,0xFF);
		  }
		  
		  GPIO_Output_Clear(&LED0_port, LED0_pin);
	      GPIO_Output_Clear(&SD_CS_Port,SD_CS_Pin);  // Clear nCS = 0
	      error_flag=Send_Command(SD_Port,CMD0,0);
	      if(error_flag==no_errors)
	      {
		      error_flag=Receive_Response(SD_Port,1,valsout);
		      GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);  // Set nCS = 1
		      GPIO_Output_Set(&LED0_port, LED0_pin);  //GREENLED=1;
	      ...
	      else
	      {
		      GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);  // Set nCS = 1
		      GPIO_Output_Set(&LED0_port, LED0_pin);  //GREENLED=1;
		      GPIO_Output_Clear(&LED3_port, LED3_pin); //REDLED=0;   indicates error has occured.
	      }
		  if(valsout[0]!=0x01)
		  {
			  ...
			  for(i=0;i<530;i++)
			  {
				  error_flag=SPI_Receive(SD_Port,&SPI_return);
			  }
			  GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);
		  }
		  timeout++;
   ...
   {
     GPIO_Output_Clear(&LED0_port, LED0_pin);  //GREENLED=0;
     Copy_String_to_Buffer(SD_Card_CMD8,0,prnt_bffr);
     UART_Transmit_String(&UART1,0,prnt_bffr);
     GPIO_Output_Clear(&SD_CS_Port,SD_CS_Pin);   // Clear nCS = 0
     error_flag=Send_Command(SD_Port,CMD8,0x000001AA);
	 if(error_flag==no_errors)
     {
        error_flag=Receive_Response(SD_Port,5,valsout);
		GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);   // Set nCS = 1
		GPIO_Output_Set(&LED0_port, LED0_pin);  //GREENLED=1;
	    Copy_String_to_Buffer(SD_Card_Resp,0,prnt_bffr);
	    UART_Transmit_String(&UART1,0,prnt_bffr);
   ...
   if(error_status==no_errors)
   {
     ...
     UART_Transmit_String(&UART1,0,prnt_bffr);
     GPIO_Output_Clear(&SD_CS_Port,SD_CS_Pin);  // Clear nCS = 0
     error_flag=Send_Command(SD_Port,CMD58,0);
	 if(error_flag==no_errors)
     {
        error_flag=Receive_Response(SD_Port,5,valsout);
		GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);  // Set nCS = 1
   ...
   if(error_status==no_errors)
   {
     ...
     UART_Transmit_String(&UART1,0,prnt_bffr);
     //GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);  // Clear nCS = 0
	 do
	 {
	    //P3_5=0;
		GPIO_Output_Clear(&SD_CS_Port,SD_CS_Pin);
	    error_flag=Send_Command(SD_Port,CMD55,0);
		if(error_flag==no_errors)  error_flag=Receive_Response(SD_Port,1,valsout);
		//P3_5=1;
	    if((valsout[0]==0x01)||(valsout[0]==0x00))
		   error_flag=Send_Command(SD_Port,ACMD41,argument);
	    if(error_flag==no_errors) Receive_Response(SD_Port,1,valsout);
		GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);
		timeout++;
	 ...
	 if(error_flag==no_errors)
     {
		...
		else
		{
		   ...
		   else
		   {
              ...
              UART_Transmit_String(&UART1,0,prnt_bffr);
              GPIO_Output_Clear(&SD_CS_Port,SD_CS_Pin);   // Clear nCS = 0
              error_flag=Send_Command(SD_Port,CMD16,512);
	          if(error_flag==no_errors)
              {
                  error_flag=Receive_Response(SD_Port,1,valsout);
		          GPIO_Output_Set(&SD_CS_Port,SD_CS_Pin);   // Set nCS = 1
